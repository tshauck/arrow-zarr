...
use metadata::{CompressorType, Endianness, MatrixOrder, ZarrDataType, PY_UNICODE_SIZE};
pub use zarr_read::{ZarrInMemoryArray, ZarrRead};
...
fn build_field(t: &ZarrDataType, col_name: String) -> ZarrResult<(usize, FieldRef)> {
    match t {
        ZarrDataType::Bool => Ok((1, Arc::new(Field::new(col_name, DataType::Boolean, false)))),
        ZarrDataType::UInt(s) => match s {
            1 => Ok((1, Arc::new(Field::new(col_name, DataType::UInt8, false)))),
            2 => Ok((2, Arc::new(Field::new(col_name, DataType::UInt16, false)))),
            4 => Ok((4, Arc::new(Field::new(col_name, DataType::UInt32, false)))),
            8 => Ok((8, Arc::new(Field::new(col_name, DataType::UInt64, false)))),
            _ => Err(ZarrError::InvalidMetadata("Invalid data type".to_string())),
        },
        ZarrDataType::Int(s) => match s {
            1 => Ok((1, Arc::new(Field::new(col_name, DataType::Int8, false)))),
            2 => Ok((2, Arc::new(Field::new(col_name, DataType::Int16, false)))),
            4 => Ok((4, Arc::new(Field::new(col_name, DataType::Int32, false)))),
            8 => Ok((8, Arc::new(Field::new(col_name, DataType::Int64, false)))),
            _ => Err(ZarrError::InvalidMetadata("Invalid data type".to_string())),
        },
        ZarrDataType::Float(s) => match s {
            4 => Ok((4, Arc::new(Field::new(col_name, DataType::Float32, false)))),
            8 => Ok((8, Arc::new(Field::new(col_name, DataType::Float64, false)))),
            _ => Err(ZarrError::InvalidMetadata("Invalid data type".to_string())),
        },
        ZarrDataType::FixedLengthString(s) => {
            Ok((*s, Arc::new(Field::new(col_name, DataType::Utf8, false))))
        }
...
fn build_array(buf: Vec<u8>, t: &DataType, s: usize) -> ZarrResult<ArrayRef> {
    let data = match t {
        DataType::Utf8 => ArrayData::builder(t.clone())
            .len(buf.len() / s)
    ...
    match t {
        ...
        DataType::UInt8 => Ok(Arc::new(UInt8Array::from(data))),
        DataType::UInt16 => Ok(Arc::new(UInt16Array::from(data))),
        DataType::UInt32 => Ok(Arc::new(UInt32Array::from(data))),
        DataType::UInt64 => Ok(Arc::new(UInt64Array::from(data))),
        DataType::Int8 => Ok(Arc::new(Int8Array::from(data))),
        DataType::Int16 => Ok(Arc::new(Int16Array::from(data))),
        DataType::Int32 => Ok(Arc::new(Int32Array::from(data))),
        DataType::Int64 => Ok(Arc::new(Int64Array::from(data))),
        DataType::Float32 => Ok(Arc::new(Float32Array::from(data))),
        ...
        _ => Err(ZarrError::InvalidMetadata(
            "Invalid zarr datatype".to_string(),
        )),
    }
}
...
fn decompress_lzma(chunk_data: &[u8], output: &mut [u8]) -> Result<(), std::io::Error> {
    let mut decompressor = XzDecoder::new(chunk_data);

    let mut offset = 0;
    while offset < output.len() {
        let read_bytes = decompressor.read(&mut output[offset..])?;
        if read_bytes == 0 {
            break;
        }
        offset += read_bytes;
    ...
    if offset < output.len() {
        return Err(std::io::Error::new(
            ...
            "Decompressed data is smaller than output buffer",
        ));
    }
...
fn decompress_array(
    ...
    compressor_params: Option<&CompressorType>,
) -> Vec<u8> {
...
fn move_indices_to_front(buf: &mut [u8], indices: &Vec<usize>, data_size: usize) {
    for (output_idx, data_idx) in indices.iter().enumerate() {
        buf.copy_within(
...
fn process_edge_chunk(
    ...
    let indices_to_keep = match n_dims {
        ...
        _ => {
            panic!("Edge chunk with more than 3 domensions, 3 is the limit")
        }
    };
...
impl<T: ZarrIterator> ZarrRecordBatchReader<T> {
    ...
    fn unpack_array_chunk(
        ...
        // decompress the data
        let chunk_size = chunk_dims.iter().product();
        ...
        if let ZarrDataType::FixedLengthPyUnicode(_) = meta.get_type() {
            ...
            data_size /= PY_UNICODE_SIZE;
        }

        // create the array
        let real_size = real_dims.iter().product::<usize>() * data_size;
...
impl<T: ZarrIterator> Iterator for ZarrRecordBatchReader<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(store) = self.predicate_projection_store.as_mut() {
            let predicate_proj_chunk = match store.next_chunk() {
                Some(Ok(x)) => x,
        ...
        let next_batch = match self.zarr_store.as_mut().unwrap().next_chunk() {
            Some(Ok(x)) => x,
            Some(Err(e)) => return Some(Err(e)),
            None => return None,
        };
